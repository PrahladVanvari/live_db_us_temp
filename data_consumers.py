...
def consume_index_value(index_tick: IndexValue, last_minute, queue_size: int, redis: direct_redis.DirectRedis, queue_name):
...
def consume_options_trade_msg(options_tick: EquityTrade, last_minute, queue_size: int, redis: direct_redis.DirectRedis, queue_name):
...
def consume_options_quote_msg(options_tick: EquityQuote, last_minute, queue_size: int, redis: direct_redis.DirectRedis, queue_name):
    internal_options_ticker = get_our_options_ticker(str(options_tick.symbol))
    redis.set(f'bid_price.{internal_options_ticker}', options_tick.bid_price)
    redis.set(f'bid_qty.{internal_options_ticker}', options_tick.bid_size)
    redis.set(f'ask_price.{internal_options_ticker}', options_tick.ask_price)
    redis.set(f'ask_qty.{internal_options_ticker}', options_tick.ask_size)